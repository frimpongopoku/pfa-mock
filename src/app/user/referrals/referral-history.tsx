...
const StatusBadge: React.FC<{ status: ReferralHistoryItem['status'] }> = ({ status }) => {
...
const ReferralRow: React.FC<{ referral: ReferralHistoryItem; index: number }> = ({ 
  referral, 
  index 
}) => {
  return (
    <motion.tr
      ...
      whileHover={{ x: 2 }}
    >
      <td className="px-4 py-4 text-slate-800 font-medium">{referral.friendName}</td>
      <td className="px-4 py-4">
        <StatusBadge status={referral.status} />
      </td>
      <td className="px-4 py-4 text-slate-700 text-right font-medium">
        ${referral.reward}
      </td>
    </motion.tr>
  );
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-slate-200">
              <th className="px-4 py-3 text-sm font-semibold text-slate-500">Friend's Name</th>
              <th className="px-4 py-3 text-sm font-semibold text-slate-500">Status</th>
              <th className="px-4 py-3 text-sm font-semibold text-slate-500 text-right">Reward</th>
            </tr>
          </thead>
          <tbody>
